...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('perusahaan.search') }}" method="post" id="perusahaanForm">
            @csrf
            <div class="row">
                <div class="col-12 col-md-8 col-lg-7">
                    <select class="form-select" name="perusahaanID" aria-label="Pilih Perusahaan" id="perusahaanSelect">
                        <option selected value="semua">Pilih Perusahaan</option>
            ...
            <table class="table table-striped" id="myTable">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $DataSiswa->nm_lengkap }}</td>
                        <td class="text-center">
                            {{ $DataSiswa->jk === 'L' ? 'Laki-Laki' : 'Perempuan' }}
                        </td>
                        <td class="text-center">{{ $DataSiswa->agama }}</td>
                        ...
                        <td class="text-center">
                            @if ($DataPKL)
                                @if ($DataPKL->status_pengajuan === 'diterima')
                                    {{ $DataPKL->perusahaan->nm_perusahaan }}
                                @elseif ($DataPKL->status_pengajuan === 'ditunggu')
                        ...
                        <td class="text-center">
                            <form id="deleteForm-{{ $DataSiswa->nis }}" action="{{ route('delete.siswa') }}" method="post">
                                @csrf
                                <input type="hidden" name="nis" value="{{ $DataSiswa->nis }}">
                                <button type="button" class="btn btn-danger" onclick="confirmDelete('{{ $DataSiswa->nis }}')">
...
<script>
    ...
    document.getElementById('perusahaanSelect').addEventListener('change', function() {
                // If the "Pilih Perusahaan" option is selected, submit the form automatically
                if (this.value === 'semua') {
                    document.getElementById('perusahaanForm').submit();
                }
            });
        </script>
@endsection
