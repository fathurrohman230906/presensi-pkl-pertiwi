    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table id="myTable" class="table table-striped table-bordered shadow-sm rounded" style="width:100%">
                ...
                <tbody>
                    @foreach($pembimbing as $key => $DataPembimbing)
                        <tr>
                            <td class="text-center align-middle">{{ $key + 1 }}</td>
                            <td class="text-center align-middle">{{ $DataPembimbing['nm_lengkap'] }}</td>
                            <td class="text-center align-middle">
                                {{ $DataPembimbing['jk'] == 'L' ? 'Laki-Laki' : ($DataPembimbing['jk'] == 'P' ? 'Perempuan' : '-') }}
                            </td>
                            <td class="text-center align-middle">{{ $DataPembimbing->jurusan->nm_jurusan }}</td>
                            <td class="text-center align-middle">{{ $DataPembimbing['agama'] }}</td>
                            <td class="text-center align-middle">{{ $DataPembimbing['alamat'] }}</td>
                            <td class="text-center align-middle text-capitalize">{{ $DataPembimbing['level'] }}</td>
                            <td class="text-center align-middle">
                                <form action="{{ route('edit.admin.pembimbing') }}" method="POST" style="display:inline;">
                                        @csrf
                                        <input type="hidden" name="pembimbingID" value="{{ $DataPembimbing['pembimbingID'] }}">
                                    ...
                                    <!-- Delete Button -->
                                    <form action="{{ route('delete.admin.pembimbing') }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <input type="hidden" name="pembimbingID" value="{{ $DataPembimbing['pembimbingID'] }}">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="staticBackdropLabel">Import Data Pembimbing</h1>
